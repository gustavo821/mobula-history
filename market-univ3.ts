...
export async function getMarketDataV3(
  proxies: string[],
  contracts: string[],
  ...
  for (let i = 0; i < contracts.length; i++) {
    if (RPCLimits[blockchains[i]] && pairs[i]) {
      ...
      const tokenGenesis = 12309863;
      console.log(
        JSON.stringify({
          topics: [[mintV3Event, collectV3]],
      ...
      if (await shouldLoad(contracts[i] + "-" + "market.json")) {
        ...
        await loadOnChainData({
          topics: [[mintV3Event, collectV3, swapV3Event]],
          address: pairs[i].map((pair) => pair.address),
          blockchain: blockchains[i],
  ...
  const blockToBalance: { [index: number]: bigint } = {};
  ...
  for (let i = 0; i < contracts.length; i++) {
    if (RPCLimits[blockchains[i]] && pairs[i]) {
      ...
      await new Promise((resolve) => {
        ...
        const logHistory: { [index: string]: number } = {};
        pipeline.on("data", async (data: any) => {
          const entry = data.value;
          if (entry && entry.address) {
            if (logHistory?.[entry.transactionHash] == entry.logIndex) {
              return;
            }
            logHistory[entry.transactionHash] = entry.logIndex;
            // console.log("=========================================");
            // console.log(entry, entry.value);
            const pair = pairsMap.get(entry.address.toLowerCase())!;

            if (pair) {
              const formattedEntries = formatV3EventToV2(pair, entry);
              ...
              for (const entry of formattedEntries || []) {
                ...
                if (entry.blockNumber > latestHistoryBlock.number) {
                  ...
                  for (
                    let q = latestHistoryBlock.index;
                    q < blocks.length;
                    q++
                  ) {
                    console.log("Fetching new history block", blocks[q][1]);
                    if (
                      blocks[q][1] > entry.blockNumber ||
                      q + 1 >= blocks.length
                    ) {
                      ...
                      console.log(
                        "Setting history latest block",
                        JSON.stringify(latestHistoryBlock),
                        blocks[q][1] > entry.blockNumber,
                        q + 1 >= blocks.length
                      );
                      break;
                  ...
                  console.log(
                    "Iterating " + daysPassed.length + " days passed."
                  );
                  ...
                  for (let q = 0; q < daysPassed.length; q++) {
                    ...
                    for (const newPair of pairs[i]) {
                      ...
                      } else if (!isNaN(freshPair.numberReserve as number)) {
                        console.log(
                          "Considering untracked pair",
                          JSON.stringify(
                            freshPair,
                            (key, value) =>
                              typeof value === "bigint"
                ...
                if (entry.topics[0] == swapEvent) {
                  ...
                  blockToBalance[entry.blockNumber] = reserve0;
                  ...
                  const tokenNumber =
                    pair.token0_address == contracts[i].toLowerCase()
                      ? "0"
                      : "1";
        ...
        pipeline.on("end", async () => {
          ...
          for (const blockNumber of Object.keys(blockToBalance) || []) {
            if (
              Number(blockNumber) < 0 /*||
              Math.abs(Number(blockNumber) - lastCheckedBlock) < 100*/
            ) {
              continue;
            }
            prm.push(
              new Promise(async (resolve) => {
                const nb = Number(blockNumber);

                const balance = blockToBalance[nb];

                let fetchedAlch = false;

                while (!fetchedAlch) {
                  try {
                    const provider = new Web3(
                      "wss://eth-mainnet.g.alchemy.com/v2/8BO3ArYtxLHwY7VQmGZ7yj1J8wzDKpCs"
                    );
                    const balanceAlch = await provider.eth.call(
                      {
                        to: "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
                        data:
                          "0x70a08231000000000000000000000000" +
                          "7858e59e0c01ea06df3af3d20ac7b0003275d4bf",
                      },
                      nb
                    );
                    fetchedAlch = true;
                    ...
                    console.info(
                      ...
                      } localBalance: ${String(balance)} alchBalance: ${String(
                        BigInt(balanceAlch)
                      )} ${
                        BigInt(balanceAlch) != BigInt(balance)
                          ? `DIVERGENCE ${
                              Number(BigInt(balanceAlch) - BigInt(balance)) /
                              10 ** 6
                            }`
                          : "OK"
                      }`
                    );
                    const bufferBnAlch = BigInt(balanceAlch);
                    if (bufferBnAlch != BigInt(balance)) {
                      let fdeferrf;
                    }
                  } catch (e) {
                    await new Promise((resolve) => {
          ...
          console.log(
            magenta(
              "Need to push a new lastEntry in history. (end)\n" +
                JSON.stringify(latestHistoryBlock)
            )
          );
  ...
  for (let i = 0; i < liquidity_per_blockchain.get(earliest)!.length; i++) {
    ...
    for (const blockchain of blockchains) {
      const index = fetchEntry(
        liquidity_per_blockchain.get(blockchain)!,
        liquidity_per_blockchain.get(earliest)![i][0]
      );
      if (blockchain != earliest && index !== null) {
